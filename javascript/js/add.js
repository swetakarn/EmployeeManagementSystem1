...
class Employee {
  ...
  class Store {
    ...
    /*static editEmployeeFromStore(email) {
      const employees = Store.getEmployees();
  
      
  for (var i = 0; i < employees.length; i++) {
     if(email === employees[i].email){  //look for match with email
         employees[i].name = "zzzzzzz";
           //add two
  ...
  function backToLoginPage(){
    window.location = "form.html"; // Redirecting to other page.
  }